...
function Modal(props) {
  const { selling_cost, deliver_date, deliver_time, onClose, order_id } = props;
  const [address, setAddress] = useState(selling_cost);
  ...
  const handleOrderEdit = async () => {
    setLoading(true)
    axios.put("http://localhost:8080/postItems/update", {
      id: order_id,
      item_cost: address,
      delivery_date: date,
      delivery_by: time,
      ...
      console.log(res)
      alert(`${res.data.message} for ${order_id}`);
      setLoading(false)
      onClose()
      ...
      <div className="modal-content py-4 text-left px-6">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-center pt-2">
            <button
              type="submit"
              className="px-10 font-semibold bg-Primary_Grey p-3 rounded-lg text-Base hover:bg-warn hover:text-Secondary_Red mr-2" onClick={() => handleOrderEdit()} 
              disabled={loading}
              >
...
function Edit(props) {
  ...
  return (
    <div>
      ...
      {isModalOpen && (
        <Modal
          selling_cost={props.selling_cost}
